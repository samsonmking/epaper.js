...
async function getRpi3In52(orientation?: Orientation, colorMode?: ColorMode): Promise<DisplayDevice> {
    try {
        const { Rpi3In52 } = await import('@epaperjs/rpi-3in52');
        return new Rpi3In52(orientation, colorMode);
    } catch (e) {
        throw new Error('Failed to import @epaperjs/rpi-3in52, make sure it is installed');
    }
}
